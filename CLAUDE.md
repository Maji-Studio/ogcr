...
- `npm run changeset` / `version` / `release` — Changesets release flow. Publishes to npm as `@majistudio/ogcr-design-system` (public, scoped; alpha `0.1.0`). `prepublishOnly` runs `build:lib`, so publish always rebuilds + contract-checks `dist/`.
